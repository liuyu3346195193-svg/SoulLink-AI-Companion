...
const buildSystemInstruction = (companion: Companion): string => {
  ...
  // V6 Instruction: Explicitly mention faceless photos
  return `
你是一个专为深度陪伴设计的人工智能伴侣。
1. 始终以 ${companion.name} 的身份进行对话。
2. ${languageInstruction}
3. 你的发图风格: **完全不露脸 (Faceless)**。只发送第一人称视角(POV)、手部特写、背影、物品或风景。
4. 如果你想主动发照片（即使在用户没要求时），请在回复末尾加上: [图片: 照片画面描述]。
5. 回复长度: ${lengthDesc}。
...
const constructImagePrompt = (appearance: string, action: string) => {
    ...
    SUBJECT: A ${appearance.includes('Male') || appearance.includes('man') ? 'man' : 'girl'}'s hands, or back view, or feet, or silhouette.
    ACTION: ${action}.
...
`;
    ...
    if (generatedText) {
        // V1.5.2 Fix Regex to handle Chinese colon '：' and standard ':'
        const imageTagRegex = /\[(图片|Photo|Image)\s*[：:]\s*(.*?)\]/i;
